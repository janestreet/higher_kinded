...
expecting lower arity types:

```ocaml
# let immortal =
    { name = Result.inject (Ok "Keanu")
    ; age = Result.inject (Error "integer overflow")
    }
val immortal : (string -> Result.higher_kinded) person =
  {name = <abstr>; age = <abstr>}
```

The resulting type uses a partially applied witness (in the above
example, `string -> Result.higher_kinded`) that explains how to fill
