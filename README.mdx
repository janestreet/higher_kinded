...
module Option :
  sig
    ...
    external inject :
      ('a t [@local_opt]) ->
      (('a -> higher_kinded) Higher_kinded.t [@local_opt]) @@ portable
      = "%identity"
    external project :
      (('a -> higher_kinded) Higher_kinded.t [@local_opt]) ->
      ('a t [@local_opt]) @@ portable = "%identity"
  end
```
...
module Result :
  sig
    ...
    external inject :
      (('a, 'z) t [@local_opt]) ->
      (('a -> 'z -> higher_kinded) Higher_kinded.t [@local_opt]) @@ portable
      = "%identity"
    external project :
      (('a -> 'z -> higher_kinded) Higher_kinded.t [@local_opt]) ->
      (('a, 'z) t [@local_opt]) @@ portable = "%identity"
  end
```
